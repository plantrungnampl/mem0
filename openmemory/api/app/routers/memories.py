...
async def list_memories(
    ...
    # Apply sorting if specified (restrict to allowed columns to prevent attribute access abuse)
    _ALLOWED_SORT_COLUMNS = {"content": Memory.content, "created_at": Memory.created_at}
    if sort_column:
        sort_field = _ALLOWED_SORT_COLUMNS.get(sort_column)
...
async def create_memory(request: CreateMemoryRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"error": "Memory client is not available. Check server logs for details."}
    ...
    try:
        ...
        return {"error": "Memory storage operation failed. Check server logs for details."}
...
async def delete_memories(request: DeleteMemoriesRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Memory service unavailable. Check server logs for details.")
